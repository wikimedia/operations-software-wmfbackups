...
private_dblist_path = os.path.join(mediawiki_config_path, "dblists", "private.dblist")
# full path for deleted.dblist
deleted_dblist_path = os.path.join(mediawiki_config_path, "dblists", "deleted.dblist")
...
def get_lists(
    all_dblist_path, private_dblist_path, deleted_dblist_path, filtered_tables_path
):
    """
    ...
    private_dblist = parse_db_file(private_dblist_path) + parse_db_file(
        deleted_dblist_path
    )
...
def get_private_databases(
    db, public_dbs, public_view_dbs, system_dbs, user_databases_regex
):
    """
    ...
    non_public_wikis_present = [
        d
        for d in databases
        if d not in public_dbs
...
def get_private_tables(db, private_tables, system_dbs, user_databases_regex):
    ...
    cursor.execute(query, private_tables + [user_databases_regex] + system_dbs)
    result = cursor.fetchall()
    cursor.close()
...
def column_has_private_data(conn, database, table, column):
    ...
    try:
        ...
        if int(result[0][0]) > 0:
            ...
            print("-- Found private data: {}.{} {}".format(database, table, column))
...
def update_columns(cols):
    ...
    for col in cols:
        print(
            ("-- UPDATE `{}`.`{}`" "   SET `{}` = NULL;").format(col[0], col[1], col[2])
        )
...
def main():
    ...
    private_databases_present = get_private_databases(
        db, public_dbs, public_view_dbs, SYSTEM_DBS, USER_DATABASES_REGEX
    )
    drop_databases(private_databases_present)

    print("-- Non-public tables that are present:")
    private_tables_present = get_private_tables(
        db, private_tables, SYSTEM_DBS, USER_DATABASES_REGEX
    )
    drop_tables(private_tables_present)

    print("-- Unfiltered columns that are present:")
    unfiltered_columns_present = get_unfiltered_columns(
        db, filtered_tables, public_wiki_dbs
    )
