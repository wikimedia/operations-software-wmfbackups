...
def parse_args():
    ...
    parser.add_argument(
        "--execute", "-e", help="the sql command to execute", required=True
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--shard",
        "-s",
        help="""Execute once on every host from
...
def print_resultset(result, tablefmt="psql"):
    ...
    if result is not None and result["success"]:
        if result["numrows"] > 0:
            print(
                tabulate.tabulate(
                    result["rows"], headers=result["fields"], tablefmt=tablefmt
                )
            )
        if result["numrows"] != 1:
            row_word = "rows"
        ...
        print("{} {} in set ({:.2f} sec)".format(result["numrows"], row_word, time))
    else:
        if result["errno"] and result["errmsg"]:
            print("ERROR {}: {}".format(result["errno"], result["errmsg"]))
        else:
...
def main():
    ...
    if options.host == "multi":
        result_list = WMFMariaDB.execute_many(
        ...
        if conn.connection is None:
            if conn.last_error is not None:
                ...
                print("ERROR unknown: Could not connect to {}".format(options.host))
            return
