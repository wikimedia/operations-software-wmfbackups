...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10000,
        help="How many rows to compare each time. The larger number of rows, the faster the full comparison will be, but also more aggressive. Also, there is an upper limit due to group_concat_max_len, depending on the size of the rows. Default: 10000 rows",
    )
    parser.add_argument(
        "--group_concat_max_len",
        type=int,
        default=100000000,
        help='Sets group_concat_max_len. Normally needed when increasing the "step" or the rows are very large. Default: 100MB',
    )
    parser.add_argument(
        "--from-value",
        type=int,
        dest="from_value",
        help="Start from given column value. Useful if only part of the table has to be scanned. Defaults to min column value for the table on the first given instance, at start.",
    )
    parser.add_argument(
        "--to-value",
        type=int,
        dest="to_value",
        help="Start from given column value. Useful if only part of the table has to be scanned. Defaults to max column value for the table on the first given instance, at start.",
    )
    parser.add_argument(
        "--order-by",
        dest="order_by",
        help="Ordering when doing the comparions. Useful when the column is not an unique field, and results could be returned out of order. By default, we order by column, which could give false positives if the column is not unique.",
    )
    parser.add_argument(
        "--print-every",
        type=int,
        dest="print_every",
        default=100,
        help="How often output should print progress report. Default: 100 queries/chunks. Set to 0 to disable progress reporting.",
    )
    parser.add_argument(
        "--threads",
        type=int,
        default=1,
        help="Parallelize requests on the given number of connections per server. By default, only 1 connection is used per server.",
    )
    parser.add_argument(
        "--verbose",
        dest="verbose",
        action="store_true",
        help="Enables verbose logging, where all SQL commands sent to the server are sent to the standard output. Defaults to disabled.",
    )
    parser.add_argument(
        "hosts",
        nargs="+",
        help="2 or more database instances, expresed with format host[:port]",
    )
...
def main():
    ...
    for result in results:
        if not result["success"]:
            print(
                "ERROR: Minimum/maximum id could not be retrieved for {}, exiting.".format(
                    result["host"] + ":" + str(result["port"])
                )
            )
            sys.exit(-1)
    ...
    try:
        for lower_limit in range(min_id, max_id + 1, options.step):
            ...
            command = "SELECT crc32(GROUP_CONCAT({4})) FROM {0} WHERE {1} BETWEEN {2} AND {3} ORDER BY {5}".format(
                options.table,
                options.column,
                lower_limit,
                upper_limit,
            ...
            for i in range(1, len(results)):
                if not (
                    results[0]["success"]
                    and results[i]["success"]
                    and results[0]["rows"][0][0] == results[i]["rows"][0][0]
                ):
                    # chunk detected as different
                    print(
                        "DIFFERENCE on {}: WHERE {} BETWEEN {} AND {}".format(
                            connections[i].host + ":" + str(connections[i].port),
                            options.column,
                            lower_limit,
                            upper_limit,
                        )
